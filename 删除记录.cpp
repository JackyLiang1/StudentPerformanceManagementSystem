...
void Delete()
{
	const studentData deletemark = { 0,"!\0", {-2,-1,-1,-1}, {-1,-1,-1,-1} };
	studentData student;
	char item;
	char subject;
	char choice;
	...
	int rank[4] = { 0 };
	string filename;
	cout << "请输入进行记录删除的文件名类型。1、默认文件“c:\\studentsFile.dat” 2、自命名 Q/q:退出\n";
	cin >> choice;
	...
	fdat.seekg(0, ios::beg);
	cout << "请输入删除操作的检索项目。1、学号 2、姓名 3、名次 Q/q:退出\n";
	cin >> item;
	switch (item)
	{
	case'1':
